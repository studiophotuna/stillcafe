 ...
 * supabase/migrations). Server-only: uses the project's secret key and the
 * workforce_snapshot / workforce_apply functions, which only that key may call.
 */
...
export const dbConfigured = () => !!(process.env.SUPABASE_URL && process.env.SUPABASE_SECRET_KEY);
